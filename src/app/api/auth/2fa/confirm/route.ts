 ...
 * Verify TOTP and complete 2FA setup for non-admin users.
 * Returns plaintext recovery codes (shown exactly once).
 */
...
export async function POST(req: NextRequest) {
  ...
  if (!token || !/^\d{6}$/.test(token)) {
    return NextResponse.json(
      { error: 'Invalid token format. Enter the 6-digit code.' },
      { status: 400 },
    );
  }
